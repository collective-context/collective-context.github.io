...
import { Aside, FileTree, LinkCard, Steps } from '@astrojs/starlight/components';
...
<Steps>
1. [ZED installieren](https://zed.dev/download) — kostenlos, Open Source (GPL)
2. Agent Panel öffnen: `Ctrl+?`
3. `+` klicken → **Claude Agent** aus ACP Registry auswählen
4. `/login` → OAuth mit claude.ai (Claude Max Abo)
</Steps>
...
<Aside type="tip" title="Boris Cherny, Creator of Claude Code">
„Anytime we see Claude do something incorrectly we add it to the CLAUDE.md, so Claude knows not to do it next time. Every mistake becomes a rule."
</Aside>
...
<LinkCard title="ZED ACP Setup Guide" href="/zed/acp" description="Agent Client Protocol — das LSP für AI-Agenten." />
<LinkCard title="Claude Code Tab im Detail" href="/zed/claude-code-tab" description="Hooks, MCP, Sub-Agents, Inline Diffs — alle Features erklärt." />
<LinkCard title="CLAUDE.md Best Practices" href="/cc/claude-md" description="Das Projekt-Gedächtnis strukturieren — Konventionen und Anti-Patterns." />
<LinkCard title="Dual-Agent Pattern" href="/patterns/dual-agent" description="Gemini CLI + Claude Code — Scanner und Fixer im Tandem." />
<LinkCard title="Case Studies" href="/case-studies/" description="Echte Workflows aus der Praxis — was funktioniert, was nicht." />
