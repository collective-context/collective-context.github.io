...
<Tabs>
  <TabItem label="macOS / Linux / WSL">
    ```bash title="Empfohlen — mit auto-updates"
    curl -fsSL https://claude.ai/install.sh | bash
    ```

    ```bash title="Erster Start"
    cd dein-projekt
    claude
  ...
  <TabItem label="Homebrew">
    ```bash title="Homebrew — kein auto-update"
...
<Tabs>
  <TabItem label="Terminal">
    ...
    ```bash title="Langen Task starten"
    claude "Refaktoriere das gesamte Auth-Modul auf JWT, schreibe Tests, erstelle PR"
    ```

    ```bash title="Session ins Web übertragen"
    claude --teleport   # → öffnet laufende Session im Browser, auf jedem Gerät
    ```
  </TabItem>
  <TabItem label="Web (claude.ai)">
    Session History in der Sidebar, strategische Planung ohne lokale Dateien.
...
## Eigene TUI-IDE: hufi-tui.py

Das [Postbox Pattern](/cc/postbox-pattern) macht Claude Code orchestrierbar — auch ohne GUI. `hufi-tui.py` aus dem [fb-data-Projekt](https://github.com/edikte/fb-data) ist ein Open-Source Session-Browser im mutt-Stil, der den Claude Code Terminal-Workflow zur vollständigen IDE erweitert:
...
```bash
# Starten
python scripts/bin/hufi-tui/hufi-tui.py
...
<LinkCard title="Praxis-Patterns" href="/terminal/patterns" description="Solo-Agent, Dual-Agent, Review-Pipeline, CI/CD — konkrete Workflow-Beispiele aus der Praxis." />
<LinkCard title="Tool-Vergleich 2026" href="/terminal/tool-comparison" description="Claude Code vs. Gemini CLI vs. Aider vs. Codex CLI — Stärken, Kosten, Einsatzszenarien." />
