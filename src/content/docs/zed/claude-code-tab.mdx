...
- **`hufi-tui.py`** — mutt-artiger Terminal-Browser: Threads lesen, umbenennen, annotieren
- **`claude_memory.py`** — CLI: list, threads, read, backup, delete

## Bekannte Limitierungen

<Aside type="caution" title="Known Issues (Stand Feb 2026)">
- Kein Context-Window-Indicator im Panel ([GitHub Discussion #49472](https://github.com/zed-industries/zed/discussions/49472))
- `/compact` noch nicht als Slash-Command verfügbar — Workaround: „Schreibe ein Handover-Dokument für die nächste Session"
- Agent Teams (mehrere Claude-Instanzen pro Session) noch nicht unterstützt
</Aside>

## Nächste Schritte

<LinkCard title="Gemini CLI hinzufügen" href="/zed/gemini-cli" description="Gemini CLI als zweiten ACP-Agenten einrichten — für den Dual-Agent Workflow." />
<LinkCard title="CLAUDE.md Best Practices" href="/cc/claude-md" description="Den Shared Context optimal strukturieren — Konventionen und Anti-Patterns." />
<LinkCard title="Dual-Agent Pattern" href="/patterns/dual-agent" description="Gemini CLI als Scanner + Claude Code Tab als Fixer koordinieren." />
