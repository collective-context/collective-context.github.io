...
CC folgt einer **3-Schichten-Architektur** für saubere Installation und Updates:

#### Layer 1: Core (via pipx)
**CCC Commander selbst** - Isoliert, sauber, konfliktfrei
```bash
pipx install cccmd
```

#### Layer 2: KI-Tools (gemischt)
- **Aider** (via pipx) - Open Source Champion mit 35.200+ Stars
- **Claude Code CLI** (via npm) - Offizielles Anthropic Tool
- **Gemini CLI** (via npm) - Offizielles Google Tool

#### Layer 3: System-Tools (via apt)
Meta-Pakete für verschiedene Deployment-Szenarien:
```bash
ccc-minimal     # Nur Commander
ccc-standard    # Commander + empfohlene Tools
ccc-full        # Komplette Suite
ccc-enterprise  # Enterprise Features + LXC
```

### Support-Modell: Debian + LXC Fokus
...
**Community-Support:**
- Ubuntu (via PPA), RHEL/Fedora (RPMs), Arch (AUR)
- macOS (Homebrew), Windows (WSL2)

### Die Vision: Ein Command für alles
```bash
ccc update --all     # Alle Tools auf einmal updaten
ccc status           # Übersicht aller installierten Tools
ccc setup --dev      # Entwickler-Setup
ccc setup --prod     # Production-Setup
```

**Problem gelöst:** Kein Update-Chaos mehr zwischen pipx, npm, apt und VS Code Extensions!
