...
<Steps>
1. **JAIL-Regel in AGENTS.md konfigurieren** — jedes Repo braucht eine klare JAIL-Regel.

   Für das Repo mit erweitertem Lesezugriff (schreibt in beide, git nur im eigenen):
...
2. **Postbox im Koordinations-Repo anlegen** — das Repo das committed und pusht:
...
3. **Zwei-Instanzen-Workflow in beiden AGENTS.md dokumentieren:**
...
**Postbox-Task bleibt liegen** → github.io-Agent wurde nicht gestartet. SysOps prüft `postbox/todo.md` am Session-Start — offene Tasks mit Fällig `sofort` werden zuerst abgearbeitet.

**Zu viele unkommittete Dateien** → Stau entsteht wenn der schreibende Agent selten Tasks einträgt. Lösung: einen Task pro inhaltlichem Block schreiben, nicht einen Task für alles.
</Aside>

<LinkCard title="Dual-Agent Pattern" href="/patterns/dual-agent" description="Gemini CLI als Scanner + Claude Code als Fixer — die einfachste Multi-Agent-Form." />
<LinkCard title="Postbox Pattern" href="/cc/postbox-pattern" description="todo, done, cron und Attachments — der vollständige Postbox-Standard." />
