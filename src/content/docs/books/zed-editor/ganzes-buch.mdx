...
> **`[o]` Open** — `/resume <uuid>` in die Zwischenablage: in der TUI surfen, den Resume-Link direkt in einen laufenden Claude Code einfügen — was ZED's History-Panel (noch) nicht kann.
